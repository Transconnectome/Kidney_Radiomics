...
# custom functions 
from utils.data_preprocessing_phase import *
...
def argument_setting(): 
    ...
    parser.add_argument("--CT_phase", default=['A', 'N', 'D', 'P'],required=False, nargs="*", help="CT phases to use", choices=["A", "D", "N", "P"])
    ...
    parser.add_argument("--scoring", default='accuracy', type=str, help="", choices=['accuracy', 'balanced_accuracy','roc_auc'])
...
if __name__ == '__main__':
    ...
    for phase in args.CT_phase:
        ...
        dataset = loading_datasets_OnePhase(save_Dir=args.save_Dir, CT_phase=phase)
        train_subject_list, test_subject_list = assign_train_test_BinaryClassification(meta_data=meta_data, train_ratio=args.train_ratio)
        ...
        k_best_value = []
        k_best_params = []
        k_best_value.append(study.best_trial.value)
        k_best_params.append(study.best_trial.params)
        for trials in study.get_trials:
            while len(k_best_params) == args.k_best: 
        """
        # saving all the best models 
        k_best_value = []
        k_best_params = []
        for k_best_trial in study.best_trials: 
            k_best_params.append(k_best_trial.params) 
        ...
        save_result(summary=summary, model=best_params, save_Dir=args.save_Dir, args=args, CT_phase=phase)
