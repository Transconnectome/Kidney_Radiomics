...
if __name__ == '__main__':
    ...
    for phase in args.CT_phase:
        ...
        print(f'Train: Oncocytoma ({train_y.squeeze(-1).tolist().count(0)}), AML ({train_y.squeeze(-1).tolist().count(1)}), chRCC ({train_y.squeeze(-1).tolist().count(2)}), pRCC ({train_y.squeeze(-1).tolist().count(3)}), ccRCC({train_y.squeeze(-1).tolist().count(4)}) \nTest: Oncocytoma ({test_y.squeeze(-1).tolist().count(0)}), AML ({test_y.squeeze(-1).tolist().count(1)}), chRCC ({test_y.squeeze(-1).tolist().count(2)}), pRCC ({test_y.squeeze(-1).tolist().count(3)}), ccRCC({test_y.squeeze(-1).tolist().count(4)}) ')
        ...
        save_result(summary=summary, model=best_params, save_Dir=f'{args.save_Dir}/subtype', args=args, CT_phase=phase)
