...
def assign_train_test(meta_data:pd.DataFrame, train_ratio=0.7): 
    # encoding as number 
    ...
    more_than2 = meta_data[meta_data['CT phase'] >= 3]
    less_than2 = meta_data[meta_data['CT phase'] < 3]
    train_subject_more_than2, test_subject_more_than2 = train_test_split(more_than2['subjectkey'].values, train_size=train_ratio,stratify=more_than2['Pathology'])
    
    # assign CT_phase < 3 only to test set 
    train_subject = copy.copy(train_subject_more_than2)
    test_subject = np.concatenate([test_subject_more_than2, less_than2['subjectkey'].values])
...
def preparing_dataset(meta_data, dataset, train_subject_list, test_subject_list): 
    meta_data = meta_data[['subjectkey', 'Pathology']]
    ...
    for phase in dataset.keys(): 
        ...
        features_train = dataset_train.drop(['subjectkey', 'Pathology'], axis=1).values       # get radiomics features of train subjects as numpy array 
        train_X.append(features_train)
        labels_train = dataset_train['Pathology'].values       # get labels of train subjects as numpy array 
        ...
        features_test = dataset_test.drop(['subjectkey', 'Pathology'], axis=1).values       # get radiomics features of test subjects as numpy array
        test_X.append(features_test)
        labels_test = dataset_test['Pathology'].values       # get labels of train subjects as numpy array 
