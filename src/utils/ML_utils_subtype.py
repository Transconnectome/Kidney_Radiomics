    ...
    if sampling_method == 'oversampling': 
        for train, val in cv.split(train_X, train_y):
            ...
            if scoring == 'roc_auc':
                ...
                pred_proba = model.predict_proba(train_X[val])
                scores.append(roc_auc_score(train_y[val], pred_proba, multi_class='ovo'))
...
def hyperparameter_tuning_selection(trial: optuna.trial.Trial, train_X, train_y, model_name='XGBoost', cv=None, min_features_to_select=1, sampling_method=None, scoring='roc_auc',seed=1234):
    ...
    sfs = SequentialFeatureSelector(model, scoring=scoring,n_features_to_select='auto', direction='backward', tol=10, n_jobs=-1)
    ...
    if sampling_method:
        ...
        if scoring == 'roc_auc':
            ...
            scores = cross_val_score(model, train_X_selected2, train_y, cv=cv, scoring='roc_auc_ovo')
        elif scoring == 'accuracy': 
...
def getting_result(estimator, test_X, test_y, threshold=0.8):
    ...
    AUC = roc_auc_score(test_y, pred_proba, multi_class='ovo')
